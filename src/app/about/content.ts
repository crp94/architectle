...
export const COVERAGE_GATES: CoverageGateRow[] = [
  ...
  },
  {
    ...
    label: {
      en: 'Max buildings per architect (still a hard gate)',
      es: 'Máximo de edificios por arquitecto (sigue siendo obligatorio)',
      it: 'Massimo di edifici per architetto (ancora vincolante)',
    },
    target: '≤6',
    actual: '3',
  },
...
export const ABOUT_SECTIONS: AboutSection[] = [
  ...
  },
  {
    id: 'v2-refocus',
    heading: {
      en: 'What changed in v2: a smaller game, the same archive',
      es: 'Qué cambió en la v2: un juego más pequeño, el mismo catálogo',
      it: 'Cosa è cambiato nella v2: un gioco più piccolo, lo stesso catalogo',
    },
    paragraphs: [
      {
        en: "As of v2, the daily puzzle, unlimited mode and the autocomplete/roster rejection no longer draw from the whole pool described above. They draw only from a hand-picked featured roster of roughly forty world-famous architects, each represented by two to four of their own buildings, selected by name recognition rather than by the coverage math below. The reason is guessability, plainly: a pool assembled to hit representativeness targets kept producing rounds whose correct answer was an architect almost nobody outside the field had heard of, which turns six guesses into trivia roulette rather than deduction. The archive you are reading right now — every building and architect page, the movement pages, the numbers on this page — is untouched by this change and still covers the full pool.",
        es: 'A partir de la v2, el rompecabezas diario, el modo ilimitado y el rechazo de autocompletado/lista de arquitectos ya no se nutren de todo el catálogo descrito arriba. Se nutren solo de una selección curada a mano de unos cuarenta arquitectos de fama mundial, cada uno representado por dos a cuatro de sus propios edificios, elegidos por su reconocimiento de nombre y no por las matemáticas de cobertura de más abajo. El motivo es la adivinabilidad, dicho sin rodeos: un catálogo ensamblado para cumplir objetivos de representatividad seguía produciendo rondas cuya respuesta correcta era un arquitecto que casi nadie fuera del gremio conocía, lo que convierte seis intentos en una ruleta de trivial en vez de una deducción. El archivo que estás leyendo ahora mismo —cada ficha de edificio y de arquitecto, las páginas de movimientos, las cifras de esta misma página— no se ve afectado por este cambio y sigue cubriendo el catálogo completo.',
        it: "A partire dalla v2, il rompicapo quotidiano, la modalità illimitata e il rifiuto dell'autocompletamento/elenco degli architetti non attingono più all'intero catalogo descritto sopra. Attingono solo a una selezione curata a mano di circa quaranta architetti di fama mondiale, ciascuno rappresentato da due a quattro dei propri edifici, scelti per la loro notorietà e non secondo la matematica di copertura qui sotto. Il motivo è, detto chiaramente, la indovinabilità: un catalogo assemblato per rispettare obiettivi di rappresentatività continuava a produrre round la cui risposta corretta era un architetto che quasi nessuno al di fuori del settore conosceva, il che trasforma sei tentativi in una roulette da quiz invece che in una deduzione. L'archivio che stai leggendo in questo momento — ogni scheda di edificio e di architetto, le pagine dei movimenti, i numeri di questa stessa pagina — non è toccato da questo cambiamento e continua a coprire l'intero catalogo.",
      },
      {
        en: "This is disclosed here rather than left for a curious reader to notice on their own: this project dropped its own representativeness targets as hard gates, on purpose, in favour of a game people can actually play. It did not drop the measurement. The era floors, the five geography rules, the 20% gender-or-non-binary minimum and the 60% canon-tier minimum — src/scripts/validators/coverage.ts, if you want to read the code itself — are still computed and printed on every single run of the curation pipeline; a curator still sees exactly where the pool stands against every one of them. What changed is that a shortfall against any of those four rule families no longer blocks a change by itself. The one distributional rule that stayed a hard gate is the cap on buildings credited to a single architect — raised from three to six, but still enforced — because that cap protects the game's answer-key distribution directly, and it matters more, not less, now that daily and unlimited draw from a much smaller featured pool.",
        es: 'Esto se explica aquí en lugar de dejar que un lector atento lo descubra por su cuenta: este proyecto abandonó sus propios objetivos de representatividad como barreras obligatorias, a propósito, en favor de un juego que la gente pueda jugar de verdad. No abandonó la medición. Los mínimos de época, las cinco reglas de geografía, el mínimo del 20% de arquitectas o personas no binarias y el mínimo del 60% de nivel «canon» —src/scripts/validators/coverage.ts, para quien quiera leer el propio código— se siguen calculando e imprimiendo en cada ejecución del proceso de curación; un curador sigue viendo exactamente en qué punto está el catálogo respecto a cada uno de ellos. Lo que cambió es que un déficit en cualquiera de esas cuatro familias de reglas ya no bloquea un cambio por sí solo. La única regla distributiva que siguió siendo una barrera obligatoria es el tope de edificios acreditados a un mismo arquitecto —elevado de tres a seis, pero aún vigente— porque ese tope protege directamente la distribución de respuestas del juego, y ahora importa más, no menos, cuando el modo diario y el ilimitado se nutren de un catálogo destacado mucho más pequeño.',
        it: "Questo viene dichiarato qui invece di lasciare che un lettore attento lo scopra da solo: questo progetto ha abbandonato di proposito i propri obiettivi di rappresentatività come soglie vincolanti, a favore di un gioco che le persone possano davvero giocare. Non ha abbandonato la misurazione. Le soglie minime per epoca, le cinque regole geografiche, il minimo del 20% di architette o persone non binarie e il minimo del 60% di livello «canone» — src/scripts/validators/coverage.ts, per chi volesse leggere il codice stesso — vengono ancora calcolati e stampati a ogni singola esecuzione della pipeline di curazione; un curatore vede ancora esattamente a che punto si trova il catalogo rispetto a ciascuno di essi. Ciò che è cambiato è che una carenza in una qualsiasi di queste quattro famiglie di regole non blocca più da sola una modifica. L'unica regola distributiva rimasta una soglia vincolante è il limite di edifici accreditati a un singolo architetto — alzato da tre a sei, ma ancora applicato — perché quel limite protegge direttamente la distribuzione delle risposte del gioco, e conta di più, non di meno, ora che le modalità quotidiana e illimitata attingono a un catalogo in evidenza molto più piccolo.",
      },
      {
        en: "Nothing about the v1 findings below changed either. The Freedom-of-Panorama exclusions, the real architects with no freely-licensed photograph findable anywhere, and the attribution-pressure risk a gender quota creates against a single-architectId answer key are all findings about the full 221-building, 237-architect archive's own curation history, not about whichever forty names the game happens to quiz you on today. They stay published in full below, exactly as before.",
        es: 'Tampoco cambió nada de los hallazgos de la v1 que siguen más abajo. Las exclusiones por Libertad de Panorama, los arquitectos reales sin ninguna fotografía con licencia abierta localizable, y el riesgo de presión de atribución que crea una cuota de género frente a una única respuesta correcta por architectId son hallazgos sobre la historia de curación de todo el catálogo de 221 edificios y 237 arquitectos, no sobre los cuarenta nombres concretos que el juego te pregunte hoy. Siguen publicados íntegramente más abajo, exactamente igual que antes.',
        it: "Nemmeno i riscontri della v1 qui sotto sono cambiati. Le esclusioni per Libertà di Panorama, gli architetti reali privi di qualsiasi fotografia con licenza aperta rintracciabile, e il rischio di pressione sull'attribuzione che una quota di genere crea nei confronti di un'unica risposta corretta per architectId sono tutti riscontri sulla storia di curazione dell'intero catalogo di 221 edifici e 237 architetti, non sui quaranta nomi specifici su cui il gioco ti interroga oggi. Restano pubblicati per intero qui sotto, esattamente come prima.",
      },
    ],
  },
  {
  ...
  },
  {
    id: 'coverage-targets',
    heading: {
      en: 'The coverage targets, now reported rather than gated',
      es: 'Los objetivos de cobertura, ahora informados en vez de exigidos',
      it: 'Gli obiettivi di copertura, ora riportati invece che vincolanti',
    },
    paragraphs: [
      {
        en: 'A pool built by taking whatever Wikidata happens to have is not neutral: measured directly against Wikidata’s own numbers, 20th-century buildings with a named architect and an image outnumber the entirety of 1000–1500 roughly sixteen to one, and a search on Frank Lloyd Wright returns 283 attributed buildings against ten for Lina Bo Bardi, three for Balkrishna Doshi and zero for Minnette de Silva. That is why this project set the numeric targets below in the first place, and through v1 they were hard failures: a change that missed one did not ship. As of v2 (see “What changed in v2” above), the era, geography, gender and canon-tier rows below are computed and printed on every run of npm run data:curate but no longer block one by themselves — only the max-per-architect row and the pool’s structural/correctness rules still do. The actual, currently-measured pool — assembled under the old hard-gate regime and unchanged by this policy shift — is shown alongside each target below exactly as before.',
        es: 'Un catálogo construido tomando lo que Wikidata tenga a mano no es neutral: medido directamente contra los propios números de Wikidata, los edificios del siglo XX con arquitecto nombrado e imagen superan a todo el periodo 1000–1500 en una proporción aproximada de dieciséis a uno, y una búsqueda sobre Frank Lloyd Wright devuelve 283 edificios atribuidos frente a diez de Lina Bo Bardi, tres de Balkrishna Doshi y cero de Minnette de Silva. Por eso este proyecto fijó en su día los objetivos numéricos de más abajo, y durante la v1 fueron fallos duros: un cambio que incumplía uno no se publicaba. A partir de la v2 (véase «Qué cambió en la v2» más arriba), las filas de época, geografía, género y nivel «canon» de más abajo se calculan e imprimen en cada ejecución de npm run data:curate, pero ya no bloquean un cambio por sí solas —solo lo siguen haciendo la fila del máximo por arquitecto y las reglas estructurales o de corrección del catálogo. La medición real y actual del catálogo —ensamblado bajo el antiguo régimen de barreras obligatorias y sin alterar por este cambio de política— se muestra junto a cada objetivo, exactamente igual que antes.',
        it: "Un catalogo costruito prendendo ciò che Wikidata ha a disposizione non è neutrale: misurato direttamente contro i numeri di Wikidata stessa, gli edifici del XX secolo con architetto nominato e immagine superano l'intero periodo 1000–1500 in un rapporto di circa sedici a uno, e una ricerca su Frank Lloyd Wright restituisce 283 edifici attribuiti contro dieci di Lina Bo Bardi, tre di Balkrishna Doshi e zero di Minnette de Silva. Per questo motivo il progetto aveva a suo tempo fissato gli obiettivi numerici qui sotto, e per tutta la v1 sono stati fallimenti bloccanti: una modifica che ne mancava uno non veniva pubblicata. A partire dalla v2 (si veda «Cosa è cambiato nella v2» sopra), le righe di epoca, geografia, genere e livello «canone» qui sotto vengono calcolate e stampate a ogni esecuzione di npm run data:curate, ma non bloccano più da sole una modifica — continuano a farlo solo la riga del massimo per architetto e le regole strutturali o di correttezza del catalogo. La misurazione reale e attuale del catalogo — assemblato sotto il vecchio regime di soglie vincolanti e non modificato da questo cambio di politica — è mostrata accanto a ogni obiettivo, esattamente come prima.",
      },
      {
        en: 'Two of these sit close enough to their floor that, back when they were still hard gates, a single future change could have broken them: 1800–1945 and 1945–2000 both came in only a few tenths of a point above their minimum at final assembly. That margin is disclosed here rather than smoothed over — and it matters slightly differently now: with these four rule families report-only, a change that erodes that margin further will show up in the numbers below without failing a build. The report exists precisely so that erosion stays visible instead of silent.',
        es: 'Dos de estos objetivos quedan lo bastante cerca de su mínimo como para que, cuando aún eran barreras obligatorias, un solo cambio futuro pudiera haberlos incumplido: 1800–1945 y 1945–2000 llegaron al ensamblaje final apenas unas décimas por encima de su mínimo. Ese margen se declara aquí en lugar de disimularlo, y ahora importa de forma ligeramente distinta: al ser estas cuatro familias de reglas solo informativas, un cambio que erosione aún más ese margen aparecerá en las cifras de más abajo sin que falle ninguna compilación. El informe existe precisamente para que esa erosión se vea, en vez de pasar en silencio.',
        it: "Due di questi obiettivi si trovano abbastanza vicini alla loro soglia minima che, quando erano ancora soglie vincolanti, un singolo cambiamento futuro avrebbe potuto farli fallire: 1800–1945 e 1945–2000 sono arrivati all'assemblaggio finale solo pochi decimi di punto sopra il minimo. Quel margine viene dichiarato qui invece di essere attenuato, e ora conta in modo leggermente diverso: essendo queste quattro famiglie di regole solo informative, una modifica che eroda ulteriormente quel margine comparirà nei numeri qui sotto senza far fallire una build. Il resoconto esiste proprio perché quell'erosione resti visibile invece che silenziosa.",
      },
    ],
